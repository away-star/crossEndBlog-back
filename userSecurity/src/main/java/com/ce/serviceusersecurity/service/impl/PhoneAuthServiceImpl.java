...
public class PhoneAuthServiceImpl implements AuthService {
    ...
    public LoginVo execute(LoginInfoRequest loginInfoRequest) {
        log.error("进来了");
        String phone = loginInfoRequest.getPhone();
        UserInfoDto initialUserInfoByPhone = securityInfoService.getInitialUserInfoByPhone(phone);
        //UserInfoDto userInfoDto = securityInfoMapper.selectUserAndPowerByPhone(phone);
        LoginVo loginVo = getLoginVo(loginInfoRequest, initialUserInfoByPhone);
